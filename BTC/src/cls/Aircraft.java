...
public class Aircraft {
	...
	private final static Sound WARNING_SOUND = audio.newSoundEffect("sfx" + File.separator + "beep.ogg");
	...
	/** Whether the aircraft has reached its destination. Note that if the destination
	 * is airport, a land command must then be given before the flight can terminate. */
	...
	private void turnBy(double angle) {
		...
		velocity = new Vector((x * cosA) - (y * sinA),
				(y * cosA) + (x * sinA),
				velocity.getZ());
	}
	...
	public void drawModifiedPath(int modified, double mouseX, double mouseY) {
		...
		if (currentRouteStage > modified - 1) {
			graphics.line(getPosition().getX(),
					getPosition().getY(),
					mouseX,
					mouseY);
		} else {
			graphics.line(route[modified - 1].getLocation().getX(),
					route[modified - 1].getLocation().getY(),
					mouseX,
					mouseY);
		}
		
		if (currentTarget == destination) {
			graphics.line(mouseX,
					mouseY,
					destination.getX(),
					destination.getY());
		} else {
			...
			if (index == route.length) { // Modifying final waypoint in route
				...
				graphics.line(mouseX,
						mouseY,
						route[index].getLocation().getX(),
						route[index].getLocation().getY());
			}
		}
	}
	 ...
	 * @return <code>true</code> if the aircraft has finished,
	 ...
	 * @param state the new altitude state: 0 = level, 1 = climbing and -1 = falling
	 */
